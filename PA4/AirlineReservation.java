...
public class AirlineReservation {
    ...
    private static void initPassengers(String fileName) throws 
        ...
        while (input.hasNext()) {
            ...
            rowContents = line.split(CSV_DELIMITER);
            planeRows = Integer.parseInt(rowContents[0]);
            firstClassRows = Integer.parseInt(rowContents[1]);
            businessClassRows = Integer.parseInt(rowContents[2]);
            break;
            }
        ...
        while (input.hasNext()) {
            String line = input.nextLine();
            rowContents = line.split(CSV_DELIMITER);
            passengers[Integer.parseInt(rowContents[0])] = rowContents[1];
        }
    }
    private static int findClass(int row) {
        if (row < 0 || row > planeRows - 1) {
            return -1;
        }
    ...
    private static int findFirstRow(int travelClass) {
        if (travelClass < 0 || travelClass > 2) {
            return -1;
        }
    ...
    public static boolean book(String passengerName, int travelClass) {
        ...
        // Business Class
            ...
            for (int i = findFirstRow(BUSINESS_CLASS); i <= findLastRow(BUSINESS_CLASS); i++) {
                if (passengers[i] == null) {
                    passengers[i] = passengerName;
                    return true;
                }
        ...
        // Economy Class
            ...
            for (int i = findFirstRow(ECONOMY_CLASS); i < planeRows; i++) {
                if (passengers[i] == null) {
                    passengers[i] = passengerName;
                    return true;
                }
